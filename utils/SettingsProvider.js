...
class RisibotSettingsProvider extends Commando.SettingProvider {
  constructor(settings) {
    super();
    this.url = `mongodb+srv://${encodeURIComponent(settings.db.user)}:${encodeURIComponent(settings.db.password)}@${encodeURIComponent(settings.db.host)}/${encodeURIComponent(settings.db.name)}?retryWrites=true&w=majority`;
  ...
  async init(client) {
    ...
    this.client = client;
    this.db = this.dbClient.db('Risibot');
    ...
    for (const user in client.users) {
      try {
        const result = await userSettingsCollection.findOne({ userId: client.users[user].id });
        let settings;

        if (!result) {
          // Can't find DB make new one.
          settings = usersettingskeys;
          userSettingsCollection.insertOne({ userId: client.users[user].id, settings });
        }
        ...
        userSettings.set(client.users[user].id, settings);
      }
      catch (err) {
        console.warn(`Error while creating document of user ${client.users[user].id}`);
        console.warn(err);
      }
    }
...
module.exports = RisibotSettingsProvider;